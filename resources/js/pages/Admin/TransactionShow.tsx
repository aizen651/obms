...
export default function TransactionShow({ transaction, lateFeeConfig }) {
    ...
    const StatusBadge = ({ status }) => {
        const config = {
            borrowed:  ['bg-blue-100 text-blue-700',  Clock],
            returned:  ['bg-green-100 text-green-700', CheckCircle2],
            overdue:   ['bg-red-100 text-red-700',     AlertCircle],
            canceled:  ['bg-gray-100 text-gray-700',   XCircle]
        };
        const [className, Icon] = config[status] || config.borrowed;
        return (
            <Badge className={`${className} border-0 flex items-center gap-1 px-3 py-1`}>
                ...
                <div className="bg-white rounded-xl shadow-md p-6 border-l-4 border-amber-500">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            {transaction.is_lost && (
                                <Badge className="bg-red-100 text-red-700 border-0 px-3 py-1">Book Lost</Badge>
                            )}
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <InfoCard icon={BookOpen} title="Book Information">
                            <div className="space-y-3">
                                <div>
                                    ...
                                    <p className="font-semibold text-lg">{transaction.book?.title || 'N/A'}</p>
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="text-sm text-gray-600">Author</label>
                                        <p className="font-medium">{transaction.book?.author || 'N/A'}</p>
                                    </div>
                                    <div>
                                        <label className="text-sm text-gray-600">ISBN</label>
                                        <p className="font-mono">{transaction.book?.isbn || 'N/A'}</p>
                                    </div>
                                </div>
                            </div>
                        </InfoCard>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <p className="font-medium capitalize">{transaction.borrower?.role || 'N/A'}</p>
                                    </div>
                                    <div>
                                ...
                                <div>
                                    ...
                                    <p className="break-all">{transaction.borrower?.email || 'N/A'}</p>
                                </div>
                                {/* Contact full width */}
                                <div>
                                    <label className="text-sm text-gray-600">Contact</label>
                                    <p>{transaction.borrower?.contact || 'N/A'}</p>
                                </div>
                            </div>
                        </InfoCard>
                            ...
                            <div className="space-y-4">
                                {[
                                    ...
                                    <div key={i} className="flex gap-3">
                                        <div className={`w-2 h-2 bg-${item.color}-500 rounded-full mt-2 flex-shrink-0`} />
                                        <div>
                                            <p className="text-sm text-gray-600">{item.label}</p>
                                            <p className={`text-${item.color}-600 font-semibold`}>{formatDate(item.date)}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                    ...
                    <div>
                        <div className="bg-white rounded-xl shadow-md p-6 sticky top-4">
                            <h2 className="text-lg font-bold mb-4 flex items-center gap-2">
                                <Edit className="w-5 h-5 text-amber-600" />Update Transaction
                            </h2>
                            <div className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        className="w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-amber-400 outline-none"
                                    >
                                        {['borrowed', 'returned', 'overdue', 'canceled'].map(s => (
                                ...
                                {state.status === 'returned' && (
                                    <div>
                                        <label className="block text-sm font-semibold text-gray-700 mb-2">
                                            <Calendar className="w-4 h-4 inline mr-1" />Date Returned
                                        </label>
                                        <input
                                            type="date"
                                            value={state.dateReturned}
                                            onChange={(e) => updateState({ dateReturned: e.target.value })}
                                            className="w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-amber-400 outline-none"
                                        />
                                    </div>
                                )}
                                ...
                                <div>
                                    <label className="block text-sm font-semibold text-gray-700 mb-2">
                                        <DollarSign className="w-4 h-4 inline mr-1" />Fees (PHP)
                                    </label>
                                    {autoEnabled && (
                                        <div className="flex items-center gap-2 mb-3 p-2 bg-amber-50 rounded border border-amber-200">
                                            <input
                                                type="checkbox"
                                                id="auto_fees"
                                                checked={state.useAutoFees}
                                                onChange={(e) => updateState({ useAutoFees: e.target.checked })}
                                ...
                                <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                                    <input
                                        type="checkbox"
                                        id="is_lost"
                                        checked={state.isLost}
                                        onChange={(e) => updateState({ isLost: e.target.checked })}
                                        className="w-4 h-4 text-amber-600 rounded"
                                    />
                                ...
                                <button
                                    ...
                                    className="w-full px-4 py-2.5 bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white font-medium rounded-lg shadow-md disabled:opacity-50"
                                >
